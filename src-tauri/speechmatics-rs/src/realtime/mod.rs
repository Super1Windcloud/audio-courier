...
impl RealtimeSession {
    ...
    async fn wait_for_start(
        ...
        channel_sender: &UnboundedSender<ReadMessage>,
    ) -> Result<()> {
        let mut retries = 0;
        let max_retries = 5;
        ...
        while !success {
            ...
            if let Some(val) = value {
                ...
                match from_slice::<models::RecognitionStarted>(&bin_data) {
                    Ok(mess) => {
                        success = true;
                    ...
                    Err(err) => {
                        ...
                        match from_slice::<models::Error>(&bin_data) {
                            Ok(mess) => {
                                return Err(Into::into(std::io::Error::new(
                                    std::io::ErrorKind::ConnectionAborted,
    ...
    pub async fn run<R: AsyncReadExt + Send + Unpin + 'static>(
        &mut self,
...
impl SenderWrapper {
    ...
    async fn send_audio<R: AsyncReadExt + Send + Unpin + 'static>(
        &mut self,
        mut reader: R,
    ) -> Result<()> {
