...
fn help_text() -> String {
    [
        "用法:",
        "  cargo run --manifest-path src-tauri/tools/Cargo.toml --bin license_tool -- generate-keypair",
        "  cargo run --manifest-path src-tauri/tools/Cargo.toml --bin license_tool -- sign --request activation_request.json --user-id customer_001 --expires-at 2027-03-27T23:59:59Z --max-version 1.9.99 --feature pro --output license.json",
    ]
    .join("\n")
}
