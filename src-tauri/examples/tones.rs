...
pub fn stream_setup_for() -> Result<cpal::Stream, String> {
...
pub fn host_device_setup() -> Result<(cpal::Host, cpal::Device, cpal::SupportedStreamConfig), String>
{
    let host = cpal::default_host();

    let device = host
        .default_output_device()
        .ok_or("Default output device is not available".to_string())?;
    ...
    T: SizedSample + FromSample<f32>,
{
    ...
    let time_at_start = time::Instant::now();
    ...
    let stream = device
        .build_output_stream(
            ...
            move |output: &mut [T], _: &cpal::OutputCallbackInfo| {
                ...
                let time_since_start = time::Instant::now()
...
fn main() {
    match stream_setup_for() {
        ...
        Err(e) => eprintln!("Error setting up stream: {e}"),
    }
}
